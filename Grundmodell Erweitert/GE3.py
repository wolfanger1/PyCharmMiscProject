...
class LagerSimulation(ShowBase):
    def __init__(self, graph_queue=None):
        ...
        self.env.draw_origin()
        self.env.erzeuge_licht()
        self.env.erzeuge_bodenraster(center_extent=70, cell_size=1)
        self.env.create_wall()


        self.annahme_stations, self.station_blue_dots = self.env.create_annahme_stations()
        self.abgabe_stations, self.abgabe_blue_dots = self.env.create_abgabe_stations()
        self.garagen_stations, self.garagen_parking_points = self.env.create_garagen_stations()
