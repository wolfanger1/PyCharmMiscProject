...
class LagerSimulation(ShowBase):
    def __init__(self, graph_queue=None):
        ...
        # **************************************************
        self.active_collisions = {}  # Schlüssel: Tuple (fahrzeug_id1, fahrzeug_id2), Wert: Kollisionsstartzeit
    ...
    def handle_to_delivery_phase(self, vehicle, dt):
        ...
        # Hole den aktuellen Auftrag
        current_order = vehicle.getPythonTag("current_order")
        if current_order is None:
            return Task.cont

        # Bestimme den Zielpunkt der Abgabestation anhand des Auftragsstrings
        ...
        # Berechne den Offset-Punkt analog zu Phase 1:
        # Hier wird der Offset (pickup_offset) in den globalen Raum transformiert und zur aktuellen Position addiert.
        current_pos = vehicle.getPos(self.render)
        offset = vehicle.getQuat(self.render).xform(self.pickup_offset)
        offset_pos = current_pos + offset

        # Prüfe, ob der Offset-Punkt nahe genug am Ziel (blauer Marker) liegt.
        if (target - offset_pos).length() < 0.5:
            print(f"Fahrzeug {vehicle.getPythonTag('vehicle_id')} hat den blauen Punkt erreicht.")
            # Fahrzeug soll stehen bleiben – Rotation erfolgt dann in Phase 7.
            vehicle.setPythonTag("order_state", "align_delivery")
            return Task.cont

        # Keine Rotation: Das Fahrzeug fährt einfach in seiner aktuellen Fahrtrichtung.
        speed = 1.5  # Basisgeschwindigkeit
        forward_vec = vehicle.getQuat(self.render).getForward()
        new_pos = current_pos + forward_vec * speed * dt
        vehicle.setPos(new_pos)
    ...
    def handle_align_delivery_phase(self, vehicle, dt):
        ...
        fork_green = vehicle.getPythonTag("fork_green")
        if not fork_green or fork_green.isEmpty():
            print("[DEBUG] handle_align_delivery_phase: Kein fork_green gefunden!")
            return

        # 2. Hole den aktuellen Auftrag und bestimme den Zielpunkt der Abgabestation.
        current_order = vehicle.getPythonTag("current_order")
        if not current_order or "ziel" not in current_order:
            print("[DEBUG] handle_align_delivery_phase: Kein aktueller Auftrag oder Ziel gefunden!")
            return
        delivery_target_str = current_order.get("ziel", "Abgabestation 1")
        try:
            target_index = int(delivery_target_str.split()[-1])
        ...
        # 3. Berechne den Pivot-Punkt wie in Phase 2 (unter Verwendung von pickup_offset).
        pivot = vehicle.getPos(self.render) + vehicle.getQuat(self.render).xform(self.pickup_offset)

        # 4. Falls der Pivot nahe am Ziel liegt, setze final die Ausrichtung.
        distance = (target - pivot).length()
        stop_threshold = 0.5  # Schwellenwert, evtl. anpassen.
        if distance < stop_threshold:
            desired_vector = target - pivot
            desired_angle = math.degrees(math.atan2(desired_vector.getY(), desired_vector.getX())) + 90
            # Zusätzlicher 45°-Offset im Gegenuhrzeigersinn (also +45°):
            desired_angle = (desired_angle + 45) % 360
            vehicle.setH(self.render, desired_angle)
            print(f"[DEBUG] final step: Fahrzeugheading auf {desired_angle:.2f}° gesetzt.")
            vehicle.setPythonTag("order_state", "delivery_center")
            return

        # 5. Berechne den gewünschten globalen Winkel und addiere 45°.
        desired_vector = target - pivot
        desired_angle = math.degrees(math.atan2(desired_vector.getY(), desired_vector.getX())) + 90
        desired_angle = (desired_angle + 45) % 360
        ...
        # 7. Begrenze die Drehung pro Frame, z. B. maximal 20,9° pro Sekunde.
        max_rotation_speed = 20.9  # Grad pro Sekunde
        max_delta = max_rotation_speed * dt
        delta_angle = max(-max_delta, min(max_delta, angle_diff))

        # 8. Drehe das Fahrzeug schrittweise um delta_angle mithilfe eines temporären Pivot-Nodes.
        temp_pivot = self.render.attachNewNode("temp_pivot")
        temp_pivot.setPos(pivot)
        self.rotate_around_pivot(vehicle, temp_pivot, delta_angle)
        temp_pivot.removeNode()
    ...
    def update_rl_on_collisions(self):
        ...
        current_collisions = self.check_collisions()  # Liefert z. B. eine Liste von (veh1, veh2)

        # Neues Dictionary, das nur die aktuell aktiven Kollisionen enthält
        new_active = {}

        for veh1, veh2 in current_collisions:
            # Erzeuge einen Schlüssel, der beide Fahrzeuge eindeutig identifiziert (Sortierung ist wichtig)
            key = tuple(sorted((veh1.getPythonTag("vehicle_id"), veh2.getPythonTag("vehicle_id"))))

            # Wenn die Kollision bereits aktiv ist, nimm den früher gespeicherten Startzeitpunkt, sonst erstelle einen neuen
            if key in self.active_collisions:
                start_time = self.active_collisions[key]
            else:
                start_time = current_time

            collision_duration = current_time - start_time

            # Beispiel: Basisreward von -10 pro Sekunde Kollisionsdauer
            reward = -10 * collision_duration

            # Update für beide beteiligte Fahrzeuge
            for veh in (veh1, veh2):
                current_pos = veh.getPos(self.render)
                min_distance = min(
                    [(current_pos - v.getPos(self.render)).length() for v in self.garage_vehicles if v != veh])
                current_speed = self.current_speed  # Oder fahrzeugspezifisch definieren
                state = [min_distance, current_speed]

                # Hier nehmen wir an, dass die letzte vom Agenten gewählte Aktion z. B. in einem PythonTag gespeichert wurde.
                last_action = veh.getPythonTag("last_braking_action")
                next_state = state  # In einem echten Szenario sollte hier der Folgezustand gemessen werden.

                self.rl_agent.update(state, last_action, reward, next_state)
                print(
                    f"RL-Update für Fahrzeug {veh.getPythonTag('vehicle_id')}: Kollisionsreward {reward:.2f} vergeben (Dauer: {collision_duration:.2f}s).")

            # Füge das Kollisionspaar mit seinem Startzeitpunkt dem neuen Dictionary hinzu
            new_active[key] = start_time

        # Ersetze das alte active_collisions-Dictionary mit nur den noch aktiven Kollisionen
