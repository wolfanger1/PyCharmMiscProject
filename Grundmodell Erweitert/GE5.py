...
class LagerSimulation(ShowBase):
    def __init__(self, graph_queue=None):
        ...
        # Hier initialisieren wir die Trainingsdaten-Liste, um künftige Samples zu speichern.
        self.training_data = []  # Diese Liste hält Tupel: (sensor_data, ideal_correction)
    ...
    def vehicle_order_task(self, vehicle, task):
        ...
        # Berechne den idealen Korrekturwert (wie bisher)
        ...
        # Führe die bestehende Phasenlogik aus – ohne eine extra Rotation hier vorzunehmen!
        if state == "translate":
    ...
    def collision_avoidance_adjustment(self, current_vehicle, base_speed, safe_distance=2.0):
        """
        Prüft für das aktuelle Fahrzeug, ob ein anderes Fahrzeug innerhalb einer
        sicheren Entfernung (safe_distance) ist, und gibt dann eine angepasste
        Geschwindigkeit zurück. Liegt ein anderes Fahrzeug zu nahe, wird die
        Geschwindigkeit proportional reduziert.
        """
        ...
        for veh in self.garage_vehicles:
            ...
            if distance < safe_distance:
                # Berechne einen Faktor (0...1), wobei bei Abstand = 0 Geschwindigkeit 0 wird.
                factor = distance / safe_distance
