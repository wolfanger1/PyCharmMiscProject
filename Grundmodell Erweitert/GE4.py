...
class LagerSimulation(ShowBase):
    ...
    def vehicle_order_task(self, vehicle, task):
        ...
        # --- Neuer Abschnitt: TensorFlow-basierte Kollisionsvermeidung und Trainingsdaten-Logging ---
        sensor_data = self.collect_sensor_data(vehicle)
        if self.tf_model is not None:
            import numpy as np
            # Annahme: Das Modell erwartet Eingabedaten in der Form (1, grid_size, grid_size, 1)
            input_data = sensor_data.reshape(1, sensor_data.shape[0], sensor_data.shape[1], 1)
            prediction = self.tf_model.predict(input_data)
            # Annahme: Das Modell gibt einen Drehwinkel in Grad zurück, den das Fahrzeug korrigieren soll.
            delta_angle = prediction[0][0]
            pivot = vehicle.getPythonTag("steering_axis")
            self.rotate_around_pivot(vehicle, pivot, delta_angle)
            print(f"[TF] Korrekturwinkel (Modell) = {delta_angle:.2f}°")
        else:
            print("[TF] Kein TensorFlow-Modell vorhanden, TF-Korrektur wird übersprungen.")

        # Wenn das Fahrzeug im Zustand "translate" ist, berechne den idealen Korrekturwert und logge das Sample.
